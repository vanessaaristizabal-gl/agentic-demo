import type { Demand, Position, Team } from './entities';
 ...
 *    posición en el equipo se fija en la vista Equipos, no en la demanda, y
...
export function positionForDemand(context: EvaluationContext): Position | undefined {
  return context.positions.find((position) => position.demandId === context.demand.id);
}
...
export const REQUIREMENTS: Requirement[] = [
  /* --- Etapa 1 · Demanda (Sales) ---------------------------------- */
  {
    id: 'intake.client',
    stage: 'demanda',
    label: 'Cliente',
    resolveIn: 'orquestacion',
    where: 'el formulario de la demanda',
    isSatisfied: ({ demand }) => filled(demand.intake.clientName),
    explain: () =>
      'La demanda todavía no tiene cliente. Escribe el nombre del cliente en el campo «Cliente» del formulario de la demanda.',
  },
  {
    id: 'intake.practice',
    stage: 'demanda',
    ...
    resolveIn: 'orquestacion',
    where: 'el formulario de la demanda',
    isSatisfied: ({ demand }) => filled(demand.intake.practice),
    explain: () =>
      'Falta elegir la práctica en el formulario de la demanda. Mientras no haya práctica, el campo «Stack tecnológico» permanece vacío y bloqueado, porque sus opciones dependen de ella.',
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el formulario de la demanda',
    isSatisfied: ({ demand }) => filled(demand.intake.costCenter),
    explain: () =>
      'El centro de costo está vacío. Es obligatorio para registrar la demanda, aunque el formulario no lo marque como tal: sin él, Finanzas no puede imputar las horas del consultor.',
  },
  ...
  /* --- Etapa 2 · Perfil (Solution Architect) ----------------------- */
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el formulario de la demanda',
    isSatisfied: ({ demand }) => filled(demand.intake.stack),
    explain: ({ demand }) =>
      filled(demand.intake.practice)
        ? 'Falta el stack tecnológico. Lo eliges en el formulario de la demanda, y solo se ofrecen los stacks de la práctica ya seleccionada.'
        : 'Falta el stack tecnológico, pero no se puede elegir mientras la demanda no tenga práctica: las opciones de stack dependen de la práctica.',
  },
  {
    ...
    isSatisfied: ({ demand }) => filled(demand.profile.seniority),
    explain: () =>
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Solution Architect',
    isSatisfied: ({ demand }) => filled(demand.profile.consultantRole),
    explain: () =>
      'Falta decir si la demanda pide un desarrollador, un QA o un tech manager. Elige el rol en el panel del Solution Architect.',
  },
  {
    id: 'profile.skills',
    stage: 'perfil',
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Solution Architect',
    isSatisfied: ({ demand }) => demand.profile.skills.length >= 3,
    explain: ({ demand }) => {
      const current = demand.profile.skills.length;
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Solution Architect',
    isSatisfied: ({ demand }) => filled(demand.profile.englishLevel),
    explain: () =>
  ...
  /* --- Etapa 3 · Equipo (Delivery Manager) ------------------------- */
  {
    ...
    isSatisfied: (context) => {
      ...
      if (!filled(demand.assignment.teamId)) return false;
      if (!teamById(context, demand.assignment.teamId)) return false;
      const position = positionForDemand(context);
      return Boolean(position) && position!.teamId === demand.assignment.teamId;
    },
    explain: (context) => {
      const { demand } = context;
      if (!filled(demand.assignment.teamId)) {
        return 'La demanda no está asignada a ningún equipo. Elige el equipo en el panel del Delivery Manager: al hacerlo se abre la posición correspondiente.';
      }
      if (!teamById(context, demand.assignment.teamId)) {
        return 'El equipo que tenía asignado la demanda ya no existe. Vuelve a asignarla desde el panel del Delivery Manager.';
      }
      return `La demanda dice estar en ${teamName(context, demand.assignment.teamId)}, pero ya no hay una posición abierta para ella en ese equipo: se liberó desde la vista Equipos. Vuelve a asignarla desde el panel del Delivery Manager.`;
    },
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Delivery Manager',
    isSatisfied: ({ demand }) => filled(demand.assignment.joinDate),
    explain: () =>
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Delivery Manager',
    isSatisfied: ({ demand }) => filled(demand.assignment.technicalReferent),
    explain: () =>
  ...
  /* --- Etapa 4 · Vacante (Recruiter) ------------------------------- */
  {
    ...
    isSatisfied: ({ demand }) => filled(demand.vacancy.title),
    explain: () =>
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Recruiter',
    isSatisfied: ({ demand }) => demand.vacancy.jobDescription.trim().length >= 120,
    explain: ({ demand }) => {
      const current = demand.vacancy.jobDescription.trim().length;
      if (current === 0) {
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Recruiter',
    isSatisfied: ({ demand }) => demand.vacancy.channels.length >= 1,
    explain: () =>
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Recruiter',
    isSatisfied: ({ demand }) => filled(demand.vacancy.salaryBand),
    explain: () =>
  ...
  /* --- Etapa 5 · Entrevista (Engineering Manager) ------------------ */
  {
    ...
    isSatisfied: ({ demand }) => filled(demand.interview.candidateName),
    explain: () =>
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Engineering Manager',
    isSatisfied: ({ demand }) => (demand.interview.technicalScore ?? -1) >= 7,
    explain: ({ demand }) => {
      const score = demand.interview.technicalScore;
      if (score === null) {
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Engineering Manager',
    isSatisfied: ({ demand }) => demand.interview.decision === 'contratar',
    explain: ({ demand }) => {
      switch (demand.interview.decision) {
        case 'segunda-ronda':
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel del Engineering Manager',
    isSatisfied: ({ demand }) => demand.interview.feedback.trim().length >= 40,
    explain: ({ demand }) => {
      const current = demand.interview.feedback.trim().length;
      return current === 0
  ...
  /* --- Etapa 6 · Onboarding (Recursos Humanos) --------------------- */
  {
    ...
    isSatisfied: ({ demand }) => filled(demand.onboarding.contractType),
    explain: () =>
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel de Recursos Humanos',
    isSatisfied: ({ demand }) => demand.onboarding.equipmentDelivered,
    explain: () =>
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel de Recursos Humanos',
    isSatisfied: ({ demand }) => demand.onboarding.accesses.length >= 3,
    explain: ({ demand }) => {
      const current = demand.onboarding.accesses.length;
  ...
  },
  {
    ...
    resolveIn: 'orquestacion',
    where: 'el panel de Recursos Humanos',
    isSatisfied: ({ demand }) => filled(demand.onboarding.buddyName),
    explain: () =>
  ...
  },
  {
    ...
    explain: (context) => {
      const { demand } = context;
      const position = positionForDemand(context);
      const team = teamName(context, demand.assignment.teamId);
      if (!position) {
        return `No hay ninguna posición abierta para esta demanda, así que no hay dedicación que fijar. Vuelve a asignar la demanda a un equipo y después fija la dedicación en la vista Equipos.`;
      }
      return `La posición de esta demanda en ${team} no tiene dedicación asignada. La dedicación no se fija aquí: ve a la vista Equipos, busca la posición abierta y asígnale un porcentaje mayor que cero.`;
    },
  },
  {
    id: 'team.capacity',
    ...
    isSatisfied: (context) => {
      const { demand } = context;
      const team = teamById(context, demand.assignment.teamId);
    ...
    explain: (context) => {
      ...
      const team = teamById(context, demand.assignment.teamId);
      if (!team) {
        return 'No se puede comprobar la capacidad porque la demanda no tiene un equipo válido asignado.';
      }
